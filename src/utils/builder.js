...
export function buildEmbed({ path, user, replacements = [], overrides }) {
  const { title, description, fields, footer, image, color } = resultFromPath(
    EMBED,
    path,
    replacements
  );
...
export function buildStringSelect({ path, options = [], replacements = [] }) {
  const { custom_id, placeholder } = resultFromPath(
    STRING_SELECT,
    path,
    replacements
  );
...
export function buildOptionList({ arr, path, replacements = [] }) {
  ...
  const appendButton = resultFromPath(
    CONFIG,
    path,
    replacements
  )
  arr.push(appendButton)
...
export function buildModal({ path, replacements = [] }) {
  const { custom_id, title, components } = resultFromPath(
    MODAL,
    path,
    replacements
  )
